...
export default function DiagnosticsScreen() {
  ...
  return <ScrollView style={styles.page} contentContainerStyle={styles.content}>
    ...
    <View style={styles.card}><Row label="ネイティブ連携" value={native.available ? '接続済み' : '未接続（Expo Go / Web）'} /><Row label="未取り込みイベント" value={String(native.pendingEventCount)} /><Row label="ネイティブ設定時給" value={native.configuredHourlyWage == null ? '未設定' : `¥${native.configuredHourlyWage}`} /><Row label="計測中セッション" value={native.activeSessions.length ? native.activeSessions.join(', ') : 'なし'} /></View>
    {!native.available && native.error ? <Text style={styles.error}>ネイティブ連携エラー: {native.error}</Text> : null}
