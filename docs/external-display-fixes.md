...
追加実機ログ `MoshiDopa-PiP-B85492DF-0B2F-4D79-BE9A-069187804E2D.json` は0.3.0 (3)、iOS 26.5.2。累積ログに旧版イベントが混在しているため、最新session `C8455ECA-47D1-4783-9534-C959847E8BFE` を分離して確認。live_activity_startedが1回成功、背景0.258秒と0.770秒の加算で合計1.028秒/¥0.514、本体前面では停止。Widgetの実描画・署名・登録情報は旧ログにないため表示不能の原因は断定できない。

親とSol Mediumの独立レビューで、request直後の「表示中」およびそのアプリ内ラベルだけを見るUIテストを偽陽性と確認。表示を「開始受付済み（OS表示は別途確認）」に変更し、診断ExportにActivityKit状態・拡張bundleID・実行ファイル存在を追加。更新完了を記録する。これらは表示成功の証明ではない。UIテストはSpringBoardの実金額アクセシビリティ表示を必須にし、失敗時にもOSスクリーンショットを残す。テスト中はstatus_bar overrideを解除し、liveactivitiesd/chronod/拡張ログを収集。OS表示不成立またはOSアクセシビリティ未公開を検出した場合、表示合格とせずログ/画像で調査する。インストール後の署名・拡張保持と、表示欠落か更新停止かの症状はユーザー確認待ち。
