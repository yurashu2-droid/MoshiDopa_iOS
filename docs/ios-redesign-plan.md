...
設計確定: 2026-09-14。本書の調査時点では実装・新CI・実機検証は未着手。以後の実装と検証結果は [ネイティブ版の検証記録](native-verification.md) を参照する。設計上の未検証事項をCI成功だけで解消済みにしない。
...
| P2A UI再現（先行） | 固定サンプルデータで紙素材・ロゴ・配色・3タブ・ホーム・履歴・設定・明細・初回案内・もしも便を再現。値札の方式選択・デザイン・面別プレビューも含む | CIの状態別画面・動画、Androidとの比較、親レビュー指摘解消。実計測を待たず確認可能 |
| P2B PiP中核PoC（並行） | 金額の連続更新・開始終了・保存・SNSとの共存 | 動画と計測ログ、実機機種/OS/署名条件付き試験結果 |
| P3 外部連携 | Live Activity、Shortcuts、表示開始導線・OS権限と設定UIの接続 | 無効化・重複・切替・ロック・終了欠落の試験結果 |
| P4 実データ統合 | 再現済みUIへ実計測・保存・移行・集計・共有を接続し、PoC結果を外部表示に反映 | fixtureテスト、実データでの画面比較、移行試験、親レビュー指摘解消 |
| P5 仕上げ | Releaseビルド、署名可能な構成、操作検証 | CI成果物、移行試験、残課題・実機手順・配布状態の報告 |

順序はP1→P2AのUI再現を先行着手し、独立したP2Bを並行する。UIは注入可能な読み取りモデルと固定fixtureで動かし、実計測やPiP実機成功を着手条件にしない。空・履歴有り・長文・大きい金額・SPEND/INVESTを再現し、見本を本番記録へ保存しない。UIの合格と背景計測・外部表示の合格を別々に記録する。

P2BでPiPの中核要件が成立しない場合、時間だけの製品に変更しない。失敗条件と公開APIの代替候補を示して製品判断に戻す。実機がない場合もP2B合格とはしないが、P2Aは継続して完成させる。PiPの実寸・OS操作との重なり・開始導線・表示可能デザインの最終確定はPoC結果に合わせる。全体の製品合格にはUI再現と計測・外部表示の双方の証拠が必要。

P2B必須試験: 本体→SNSで15分以上、連続60分、音声ON/OFF、YouTube/Instagram/TikTok/Xそれぞれ、相手PiP、ロック/復帰、通知/コントロールセンター、PiP閉じる/再開、プロセス終了、低電力、急速なSNS切替。1Hzフレーム更新の時刻ログと画面録画を突合し、ドリフト・中断・音声干渉・CPU/電池傾向を記録。電池値は端末条件を固定して比較し、未測定の省電力を宣伝しない。
...
本書と3調査書を読む。Android baselineの差分を確認し、新変更があれば勝手に基準を更新せず差分を報告する。まずP1を整備しP2AのUI再現を先行、P2BのPiP PoCを並行する。Actionsの実行結果と実機試験を区別し、P2B未検証のまま「他アプリ上で金額が動く」を達成扱いしない。UIは実機待ちで止めず、画像比較と親レビューまで完成させる。その後P3で外部連携を整え、P4で実データ統合、P5で仕上げる。実機/署名不足は具体化し、最終製品合格は未検証を残して宣言しない。今回の設計段階ではmain更新・アプリ変更・新CI実行は行わない。
