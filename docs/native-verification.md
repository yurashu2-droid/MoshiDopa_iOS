...
[オートメーションガイド検証](automation-validation.md) にRun 34912286439、親レビュー、限定再確認の理由、IPAのハッシュを記録。値札設定と実計測画面から8段階のShortcuts設定ガイドを開ける。対象名・手順の保存、アクション名のコピー、`shortcuts://` での起動、最後の実計測への遷移を実装。手順変更時は先頭へ戻る。

個人用オートメーションの登録と読み取りをアプリが完了したとは扱わない。ユーザーがShortcuts内で登録し、実機で開閉配送を確認する必要がある。署名・実機インストール、SNS開閉配送、App Store審査は未確認。
