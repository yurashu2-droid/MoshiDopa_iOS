...
## Shortcuts setup guide / 0.3.3 build 6

The counter settings and real-measurement sheet now open an eight-step, resumable guide for one target app's open and close automations. It opens Shortcuts through Apple's public URL scheme, copies the exact intent and target names on request, and returns to the selected PiP/Live Activity measurement screen. It records guide progress only and never labels an OS automation as configured. Parent reviewed the rendered final step and the integrated source; a bounded Sol Medium review found no remaining compile, state-guard or CI-selection issue.

Run 34908344216 established 19 unit passes, 12 of 13 normal UI passes, two compact passes and a Release build; its only failure was an XCTest activation point for the mode menu before measurement started. Run 34911018054 then passed real background measurement but exposed retained scrolling between guide pages. Run 34912286439 passed 19 units, guide and real measurement UI tests, Release packaging and native-gate after isolating the test prerequisite and resetting page scroll. See `automation-validation.md`. The app still requires physical-device verification of Shortcuts delivery and is not yet a completed replacement for fixture-backed normal history/data flows.
